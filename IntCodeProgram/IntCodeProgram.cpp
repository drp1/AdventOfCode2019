// IntCodeProgram.cpp : Defines the functions for the static library.
//
...
void StringToIntCodeProgram(string const& inputText, IntCodeProgram& program)
{
	...
	while (!ss.eof())
	{
		...
		} while ((isdigit(nextChar) || nextChar == '-') && !ss.eof());
...
string RunProgramWithPhaseSetting(IntCodeProgram& program, int const phaseSetting, int const inputData)
{
	string programOutput;
	bool hasEnteredPhaseSetting = false;
	...
	do
	{
		...
		case Opcode::Multiply:
			...
			break;
		case Opcode::Input:
			IntCodeInput(program, hasEnteredPhaseSetting ? inputData : phaseSetting);
			hasEnteredPhaseSetting = true;
			break;
		case Opcode::Output:
	...
	throw exception("Program exited unexpectedly");
}